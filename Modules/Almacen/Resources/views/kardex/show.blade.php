    ...
    <div class="w-full mt-3 flex flex-wrap gap-5">
        ...
        <div class="w-full p-3 shadow shadow-shadowminicard rounded max-w-sm bg-fondominicard">
            ...
            <div class="mt-3 flex flex-col gap-2">
                <div>
                    <x-label value="Serie :" />
                    <x-span-text :text="$serie->serie" />
                </div>
                <div>
                    <x-label value="Fecha registro :" />
                    <x-span-text :text="formatDate($serie->created_at)" />
                </div>
                <div>
                    <x-label value="Producto relacionado:" />
                    <x-span-text :text="$serie->producto->name" />
                </div>
                <div>
                    <x-label value="Almacén relacionado :" />
                    <x-span-text :text="$serie->almacen->name" />
                </div>
                <div>
            ...
            <div class="w-full p-3 shadow shadow-shadowminicard rounded max-w-sm bg-fondominicard">
                ...
                <div class="mt-3 flex flex-col gap-2">
                    <div>
                        ...
                        <x-span-text :text="formatDate($serie->compraitem->created_at)" />
                    </div>
                    <div>
                        <x-label value="Cantidad entrante :" />
                        <x-span-text :text="formatDecimalOrInteger($serie->compraitem->cantidad) .
                            ' ' .
                            $serie->compraitem->producto->unit->name" />
                    </div>
                    <div>
                        <x-label value="Documento referencia entrada :" />
                        <x-span-text :text="$serie->compraitem->compra->referencia" class="block" />
                        <p>
                            <a class="text-linktable hover:text-hoverlinktable text-xs underline inline-block"
                                href="{{ route('admin.almacen.compras.show', $serie->compraitem->compra->id) }}">
                                Ver compra</a>
                        </p>
                    </div>
                </div>
            ...
            <div class="w-full p-3 shadow shadow-shadowminicard rounded max-w-sm bg-fondominicard">
                ...
                <div class="mt-3 flex flex-col gap-2">
                    <div>
                        ...
                        <x-span-text :text="formatDate($serie->itemserie->date)" />
                    </div>
                    <div>
                        <x-label value="Cantidad salida :" />
                        <x-span-text :text="formatDecimalOrInteger($serie->itemserie->tvitem->cantidad) .
                            ' ' .
                            $serie->producto->unit->name" />
                    </div>

                    @if ($serie->itemserie->tvitem->tvitemable instanceof \Modules\Ventas\Entities\Venta)
                        <div>
                            <x-label value="Documento referencia salida :" />
                            <x-span-text :text="$serie->itemserie->tvitem->tvitemable->code" />
                            <a class="text-linktable hover:text-hoverlinktable text-xs underline block"
                                href="{{ route('admin.ventas.show', $serie->itemserie->tvitem->tvitemable->id) }}">
                                Mostrar venta</a>
                        </div>

                        {{-- {{ $serie->itemserie->tvitem->tvitemable }} --}}
                    @endif
