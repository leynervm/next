        ...
        <form wire:submit.prevent="save" class="w-full flex flex-col gap-2 bg-body p-3 rounded">
            <div class="w-full flex flex-col xs:grid xs:grid-cols-2 xl:grid-cols-3 gap-2">
                <div class="w-full xs:col-span-2">
                    ...
                    <div id="parentproveedorcompra_id" class="relative" x-data="{ proveedor_id: @entangle('proveedor_id').defer }" x-init="selec2Proveedor"
                        wire:ignore>
                        <x-select class="block w-full" x-ref="select" wire:model.defer="proveedor_id"
                            id="proveedorcompra_id" data-minimum-results-for-search="3">
                            <x-slot name="options">
                ...
                <div class="w-full">
                    ...
                    <div id="parentmonedacompra_id" class="relative" x-data="{ moneda_id: @entangle('moneda_id').defer }" x-init="selec2Moneda"
                        wire:ignore>
                        <x-select class="block w-full" x-ref="select" id="monedacompra_id">
                            <x-slot name="options">
                                @if (count($monedas))
                ...
                <div class="w-full" x-show="!paycuotas">
                    <x-label value="Forma pago :" />
                    <div id="parentmethodpaymentcompra_id" class="relative" x-init="select2Formapago" wire:ignore>
                        <x-select class="block w-full" x-ref="selectfp" id="methodpaymentcompra_id">
                            <x-slot name="options">
                ...
                @if (count($cuentas) > 1)
                    <div class="w-full" x-show="!paycuotas">
                        ...
                        <div id="parentcuentacompra_id" class="relative" x-init="select2Cuentas" wire:ignore>
                            <x-select class="block w-full" x-ref="select" id="cuentacompra_id">
                                <x-slot name="options">
            ...
            <x-jet-input-error for="typepayment.paycuotas" />
    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('data', () => ({
                ...
                methodpayment_id: @entangle('methodpayment_id'),
        ...
        function select2Typepayment() {
            ...
            this.selectT.on("select2:select", (event) => {
                ...
                let datapaycuotas = event.target.options[event.target.selectedIndex].getAttribute(
                    'data-paycuotas');
                this.paycuotas = datapaycuotas == '1' ? true : false;
        ...
        function select2Formapago() {
            this.selectF = $(this.$refs.selectfp).select2();
