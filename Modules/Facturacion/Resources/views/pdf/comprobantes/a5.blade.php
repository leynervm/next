...
<style>
    ...
    * {
    ...
    @page {
        margin: 1.75cm 0.25cm 3.25cm 0.25cm;
    }
...
<body>
                ...
                <tr class="align-baseline">
                    ...
                    <th class="align-baseline" style="padding: 0 2px;">
                        ...
                        @if (count($comprobante->sucursal->empresa->telephones) > 0)
                            <p class="font-normal text-9 leading-2">
                                TELÉFONO:
                                {{ formatTelefono($comprobante->sucursal->empresa->telephones->first()->phone) }}
                            </p>
                        @endif

                    </th>
                    <th class="border" style="vertical-align:middle;width:150px;">
                        <p class="font-bold text-10 leading-4">
                            {{ $comprobante->sucursal->empresa->document }}</p>
                        <p class="font-bold text-10 leading-4">
                            {{ $comprobante->seriecomprobante->typecomprobante->descripcion }}
                        </p>
                        <p class="font-bold text-10 leading-4">{{ $comprobante->seriecompleta }}</p>
                    </th>
                </tr>
            </thead>
        </table>
    ...
    @php
        ...
        $bottom = !empty($comprobante->sucursal->empresa->web) ? '-3.2cm' : '-3cm';
    @endphp
        ...
        <table class="table" style="padding: 0 2.5px; border-collapse: separate;">
            <thead>
                <tr class="align-middle">
                    <td class="border">
                        <img class="" style="padding:1mm;" src="data:image/svg;base64, {!! base64_encode(QrCode::format('svg')->size(80)->generate($hashcomprobante)) !!} ">
                    </td>
                    <td class="title border" style="">
                        <p class="font-normal leading-2 text-9" style="text-align: justify">
                        ...
                        <p class="font-normal leading-2 text-9 mt-1">
                        ...
                        <p class="text-center font-bold text-8 mt-3 leading-2">
                ...
                @if (!empty($comprobante->sucursal->empresa->web))
                    <tr>
                        <td colspan="2" class="text-center text-9 font-normal">
    ...
    <div class="body">
        <table class="table mt-1 text-9">
            <tbody>
                <tr>
                    <td class="font-medium" style="width: 80px">
                        FECHA EMISION </td>
                    <td class="p-1 font-bold">
                        : {{ formatDate($comprobante->date, 'DD/MM/Y HH:mm A') }}</td>
                </tr>
                <tr>
                    <td class="font-medium align-baseline" style="width:80px">
                        CLIENTE </td>
                    <td class="font-bold">
        ...
        @if (count($comprobante->facturableitems) > 0)
            <table class="table mt-3 text-9">
                <thead style="background: #CCC">
                    <tr class="border-table">
                        <th class="font-bold text-center" style="">ITEM</th>
                        <th class="font-bold p-1 text-center" style=";">DESCRIPCIÓN</th>
                        <th class="font-bold p-1 text-center" style=";">CANTIDAD</th>
                        <th class="font-bold p-1 text-center" style=";">P. UNIT.</th>
                        <th class="font-bold p-1 text-center" style=";">IMPORTE</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($comprobante->facturableitems as $item)
                        <tr class="border-table font-normal">
                            <td class="p-1 text-center align-middle" style="width: 30px;">
                                {{ $item->item }}
                            </td>
                            <td class="p-1 align-middle text-start leading-2">
                                {!! nl2br($item->descripcion) !!}</td>
                            <td class="p-1 text-center" style="width: 70px;">
                            ...
                            <td class="p-1 text-center align-middle" style="width: 80px;">
                                {{ number_format($item->total, 2, '.', ', ') }}</td>
                        </tr>
                    @endforeach
                    <tr>
                        <td class="font-bold p-1 text-start leading-3" colspan="2" style="">
