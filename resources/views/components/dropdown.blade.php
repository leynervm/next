<div class="relative" x-data="{ open: false }">
    <button x-on:click="open = !open" :class="{ 'bg-next-50': open, 'bg-transparent': !open }"
        class="border bg-transparent rounded-lg w-full text-xs font-semibold border-next-300 text-next-500 p-2 px-3 focus:ring-0 focus:border-next-400 text-center inline-flex items-center"
        type="button">
        {{ $titulo }}
    ...
    <div :class="{ 'block': open, 'hidden': !open }" x-show="open" x-on:click.away="open = !open"
        ...
        class="absolute hidden z-10 w-auto max-w-xs bg-fondodropdown rounded-lg sm:rounded-xl shadow-md shadow-shadowminicard">
        @if (isset($pages))
            {{ $pages }}
        @endif
