...
<div x-data="timer()" x-init="start()" class="z-[9999]">
    <div class="fixed z-50 border-stroke flex items-center rounded-xl border border-l-[6px] {{ $borderalert }} bg-fondominicard p-2 w-full max-w-sm bg-opacity-80"
        ...
        <div
            class="mr-3 p-3 flex-shrink-0 flex items-center justify-center rounded-full {{ $bgalert }} text-white">
            @if (isset($icono))
                {{ $icono }}
            @endif
        </div>
        <div class="w-full flex-1 relative">
            <h3 class="mb-1 text-sm font-semibold pr-2 {{ $colortitlealert }}">
                {{ $titulo }}</h3>
            <p class="text-textspancardproduct text-xs leading-3 font-medium">
                {{ $mensaje }}</p>
            <button class="w-4 h-4 hover:text-red-800 text-red-600 absolute top-0 right-0" @click="open = false">
