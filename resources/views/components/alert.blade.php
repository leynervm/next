...
<div x-data="timer()" x-init="start()">
    <div class="fixed z-50 border-stroke flex items-center rounded-md border border-l-[8px] {{ $borderalert }} bg-fondominicard p-3 w-full max-w-sm bg-opacity-80"
        ...
        <div class="flex w-full items-center justify-between">
            <div>
                <h3 class="mb-1 text-md font-medium {{ $colortitlealert }}">
                    {{ $titulo }}
                </h3>
                <p class="text-textspancardproduct text-xs leading-3">
                    {{ $mensaje }}
                </p>
            </div>
            <div>
                <button class="hover:text-red-400 text-gray-400" @click="open = false">
