<div x-data="importar">
    @can('admin.almacen.productos.import')
        <x-button-next titulo="Importar lista productos" wire:click="$set('open', true)">
    ...
    <x-jet-dialog-modal wire:model="open" maxWidth="2xl" footerAlign="justify-end">
        <x-slot name="title">
            {{ __('IMPORTAR LISTA DE PRODUCTOS') }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <form wire:submit.prevent="import" class="w-full flex flex-col justify-center items-center gap-2">
                <label class="">
                    ...
                    <p class="block w-fuill max-w-28 text-[10px] mt-2 text-center leading-none font-semibold">
                        @if ($file)
                            ARCHIVO CARGADO CORRECTAMENTE
                        @else
                            IMPORTAR LISTA PRODUCTOS
                        @endif
                    </p>
                </label>
