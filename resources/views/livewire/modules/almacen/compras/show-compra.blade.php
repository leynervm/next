<div>
    <div class="w-full flex flex-col gap-8" wire:key="show-compra{{ $compra->id }}">
        ...
        <x-form-card titulo="DATOS COMPRA" subtitulo="Resumen de los datos princiales de la compra.">
                ...
                <div class="w-full flex flex-col xs:grid xs:grid-cols-2 xl:grid-cols-3 gap-2">
                    ...
                    @if ($moneda->code == 'USD')
                        <div class="w-full">
                            ...
                            <x-input class="block w-full numeric" wire:model.defer="compra.tipocambio"
                    ...
                    <div class="w-full">
                        ...
                        <x-input class="block w-full numeric" wire:model.lazy="compra.igv" placeholder="0.00"
                            type="number" min="0" step="0.0001" />
            ...
            <div wire:loading.flex wire:target="update, refresh, delete" class="loading-overlay rounded hidden">
                <x-loading-next />
            </div>
        </x-form-card>
        ...
        @if ($compra->typepayment->paycuotas)
            <x-form-card titulo="CUOTAS PAGO" subtitulo="Información de cuotas de pago de la compra."
                        ...
                        <div class="w-full flex gap-2 flex-wrap justify-start">
                            @foreach ($compra->cuotas as $item)
                                <x-card-cuota class="w-full xs:w-48" :titulo="substr('000' . $item->cuota, -3)" :detallepago="$item->cajamovimiento"
                                    :wire:key="'cardcuota-'.$item->id" wire:ignore>
                                        ...
                                        @if ($item->cajamovimiento)
                                            <div class="w-full flex gap-2 flex-wrap items-end justify-between">
                                                ...
                                                <x-button-delete
                                                    ...
                                                    wire:loading.attr="disabled" :wire:key="'deletepay-'.$item->id" />
                                            </div>
                                        @else
                                            <div class="w-full flex gap-2 flex-wrap items-end justify-between">
                                                ...
                                                onclick="deletecuota({{ $item->id }})"
                                                    ...
                                                    wire:loading.attr="disabled" :wire:key="'delete-'.$item->id" />
                                            </div>
                                        @endif
                                    </x-slot>
                ...
                <div x-show="loadingcuotas" wire:loading wire:loading.flex
                    wire:target="calcularcuotas, deletecuota, deletepaycuota, savecuotas"
            ...
            <x-slot name="content">
                <form wire:submit.prevent="savepayment" class="w-full flex flex-col gap-1">
                    <div class="w-full">
                        ...
                        <x-span-text :text="'Cuota' .
                            substr('000' . $cuota->cuota, -3) .
                            ' / ' .
                            $compra->moneda->simbolo .
                    ...
                    <div class="w-full">
                        ...
                        <div class="relative">
                            <x-select class="block w-full" id="editcompramethodpayment_id"
                                wire:model.defer="methodpayment_id" data-dropdown-parent="null">
                                <x-slot name="options">
                                    ...
                                    <x-card-cuota :titulo="substr('000' . $item['cuota'], -3)" class="w-full sm:w-48">
                                        ...
                                        @if (is_null($item['cajamovimiento_id']))
                                            <x-input class="block w-full numeric" type="number" min="1"
                                                ...
                                                oninput="return notsimbols(event)" />
                                        @else
