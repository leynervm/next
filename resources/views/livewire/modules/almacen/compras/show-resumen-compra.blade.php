                    ...
                    @foreach ($compra->compraitems as $item)
                        ...
                        <x-card-producto :image="$image" :name="$item->producto->name" :discount="$discount ?? null" x-data="{ loadingproducto: false }">
                            ...
                            <div class="w-full flex flex-wrap gap-1 items-start mt-2 text-[10px]">
                                ...
                                <x-span-text :text="formatDecimalOrInteger($item->cantidad) . ' ' . $item->producto->unit->name" class="leading-3" />
                                ...
                                @if (count($item->series) == 1)
                                    <span
                                        ...
                                        <x-button-delete
                                            wire:click="$emit('compra.confirmDeleteSerie',{{ $item->series->first() }})"
                                        ...
                                        @if (count($pricetypes))
                                            <div class="w-full grid xs:grid-cols-2 lg:grid-cols-1 gap-1">
                                                @foreach ($pricetypes as $lista)
                                                    @php
                                                        $precios = \App\Helpers\GetPrice::getPriceProducto($item->producto, $lista->id, $empresa->tipocambio)->getData();
                                                        // var_dump($precios);
                                                    @endphp
