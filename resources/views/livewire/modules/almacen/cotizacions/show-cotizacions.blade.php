<div>
    <div wire:loading.flex class="loading-overlay fixed hidden" wire:key="showcotizacions">
        <x-loading-next />
    </div>
        ...
        <x-table class="w-full mt-1">
            ...
            <x-slot name="body">
                @foreach ($cotizacions as $item)
                    <tr>
                        <td class="p-2 text-[10px] text-colorsubtitleform">
                            @if ($item->trashed())
                                {{ $item->seriecompleta }}
                            @else
                                @can('admin.cotizaciones.edit')
                                    <a href="{{ route('admin.cotizacions.edit', $item) }}"
                                ...
                                @cannot('admin.cotizaciones.edit')
                        ...
                        <td class="p-2 text-center leading-none text-[10px] min-w-28">
                            Válido hasta <br>
                            {{ formatDate($item->validez, 'DD MMMM Y') }}
                        </td>
                        ...
                        <td class="p-2 text-center align-middle">
                            @if ($item->isAprobbed())
                                <x-span-text text="APROBADO" type="green" />
                            @else
                                <x-span-text text="PENDIENTE" type="orange" />
                            @endif
                        </td>
    ...
    @else
        <x-span-text text="NO EXISTEN REGISTROS DE COTIZACIONES..." class="mt-3 bg-transparent" />
    @endif
    ...
    @if ($cotizacions->hasPages())
        <div class="w-full flex justify-center items-center sm:justify-end p-1 sticky -bottom-1 right-0 bg-body">
            {{ $cotizacions->onEachSide(0)->links('livewire::pagination-default') }}
        </div>
    @endif
