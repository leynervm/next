<div wire:init="loadProductos" x-data="carshoop">
    ...
    <aside x-show="sidebar" class="sidebar-productos" x-bind:class="openSidebar ? 'lg:z-[90]' : 'z-[199]'"
        ...
        <div class="w-full relative p-2 h-full max-h-full overflow-y-auto !overflow-x-hidden">
            <div class="w-full flex justify-end items-end">
                <button class="p-2.5 font-semibold !leading-none text-colorsubtitleform"
                    @click="sidebar=false,backdrop = false">✕</button>
            </div>

            @if (count($productos) > 0)
                <x-simple-card x-data="{ openfilter: true }" wire:key="dropdownprices" class="w-full text-colorsubtitleform">
                    <button type="button"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" class="block w-6 h-6">
                    ...
                    <div class="w-full flex flex-col max-h-60 overflow-y-auto" x-cloak x-show="openfilter"
                        ...
                        @foreach ($subcategories as $item)
                            <label for="subcategory_id_{{ $item->id }}"
                                class="text-xs font-medium hover:bg-shadowminicard hover:text-textspancardproduct rounded p-2 cursor-pointer inline-flex items-center gap-1 transition-colors ease-in-out duration-150">
                                <input type="checkbox" wire:model.lazy="selectedsubcategorias" name="subcategorias[]"
                                    class="rounded p-2 outline-none cursor-pointer shadow-none shadow-0 border ring-0 focus:ring-0 hover:text-next-500 checked:text-next-500 focus:text-next-500"
            ...
            @if (count($caracteristicas) > 0)
                @foreach ($caracteristicas as $item)
                    <x-simple-card x-data="{ openfilter: false }" wire:key="dropdowncaracteristicas_{{ $item->id }}"
                        ...
                        <div class="w-full flex flex-col max-h-60 overflow-y-auto" x-cloak x-show="openfilter"
                            ...
                            @foreach ($item->especificacions as $espec)
                                <label for="especificacion_{{ $item->id }}{{ $espec->id }}"
                                    class="text-xs font-medium hover:bg-shadowminicard hover:text-textspancardproduct rounded p-2 cursor-pointer inline-flex items-center gap-1 transition-colors ease-in-out duration-150">
                                    <input type="checkbox" wire:model.lazy="especificacions" name="especificacions[]"
                                        class="rounded p-2 outline-none cursor-pointer shadow-none shadow-0 border ring-0 focus:ring-0 hover:text-next-500 checked:text-next-500 focus:text-next-500"
                                        value="{{ $espec->slug }}"
                ...
                @if ($pricesale > 0)
                    ...
                    <x-slot name="footer">
                        ...
                        <div x-cloak x-show="addcart"
                            ...
                            x-data="carshoop" x-transition:enter="opacity-0 transition ease-in-out duration-300"
                            ...
                            <div class="w-full flex-1 flex justify-center xl:justify-start gap-0.5 gap-x-3">
                                ...
                                <x-input x-model="qty" class="w-full rounded-xl flex-1 text-center text-colorlabel input-number-none"
