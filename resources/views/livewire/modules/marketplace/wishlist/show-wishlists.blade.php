            ...
            <div class="w-full">
                ...
                <div class="w-full flex flex-col gap-3">
                    @foreach ($wishlists as $item)
                        ...
                        <div class="relative w-full flex flex-col gap-2 text-xs p-1 sm:p-2 border border-borderminicard rounded-lg md:rounded-2xl"
                            wire:key="{{ $item->rowId }}">
                            <div
                                class="{{ $item->options->is_disponible ? '' : 'opacity-50' }} w-full flex flex-col xs:flex-row gap-2 text-xs">
                                <div class="w-full size-48 xs:size-32 rounded overflow-hidden">
                                    @if ($image)
                                        <img src="{{ $image }}" alt=""
                                ...
                                <div class="w-ful flex-1 flex flex-col justify-between">
                                    <div class="w-full">
                                        ...
                                        <div class="w-full flex flex-wrap gap-1 justify-end items-end">
                                            @if (count($item->options->carshoopitems) == 0)
                                                <x-button-add-car type="button" wire:loading.attr="disabled"
                                                    ...
                                                    wire:key="add_{{ $item->rowId }}">MOVER AL
                                                    CARRITO</x-button-add-car>
                                            @endif

                                            <button wire:click="deleteitem('{{ $item->rowId }}')"
