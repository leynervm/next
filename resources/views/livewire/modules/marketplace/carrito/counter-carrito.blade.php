<div class="group" x-data="{ opencounter: @entangle('opencounter'), cartOpen: false, isOpen: false }">
    <button @click="cartOpen = !cartOpen" wire:click="open"
        class="hidden xl:flex p-3 px-1 sm:px-3 w-full h-full justify-start items-center hover:opacity-80 transition ease-in-out duration-150">
    ...
    <span id="countercart"
        class="{{ Cart::instance('shopping')->count() == 0 ? 'hidden' : 'flex' }} absolute w-4 h-4 top-0.5 -right-1 xl:right-1 tracking-tight h-100 justify-center items-center leading-3 text-[9px] bg-fondobadgemarketplace text-colorbadgemarketplace rounded-full">
    ...
    {{-- @if (Cart::instance('shopping')->count() > 0)
        <div :class="cartOpen ? 'translate-x-0 ease-out' : 'translate-x-full ease-in'"
            ...
            @if (Cart::instance('shopping')->count() > 0)
                ...
                <div
                    class="p-3 mt-auto rounded-b-xl bg-next-800 w-full flex justify-between items-center flex-row gap-2">
                    <div>
                        <p class="text-[10px] text-white">TOTAL</p>
                        <p class="text-xl text-white font-semibold">
                            <small class="text-[10px] font-medium">{{ $moneda->simbolo }} </small>
