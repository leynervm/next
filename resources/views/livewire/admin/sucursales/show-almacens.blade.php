<div>
                ...
                <div class="w-full lg:w-80 xl:w-96 lg:flex-shrink-0 relative" x-data="{ savingalmacen: false }">
                    <form wire:submit.prevent="save" class="flex flex-col gap-2">
                        <div class="w-full">
                            ...
                            <x-input class="block w-full" wire:model.defer="name" placeholder="Nombre de almacén..." />
                ...
                @if (count($sucursal->almacens))
                    <div class="w-full flex flex-wrap gap-2">
                        @foreach ($sucursal->almacens as $item)
                            <x-minicard :title="null" :content="$item->name" :alignFooter="$item->default ? 'justify-between' : 'justify-end'" size="md"
                                class="bg-body">
                                <x-slot name="buttons">
                                    ...
                                    <div>
                                        ...
                                        @if (Module::isEnabled('Almacen'))
                                            <x-button-delete
                                                wire:click="$emit('sucursales.confirmDeleteAlmacen', {{ $item }})"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            Livewire.on('sucursales.confirmDeleteAlmacen', data => {
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        Livewire.emitTo('admin.sucursales.show-almacens', 'delete', data.id);
                    }
                })
            })
        })
