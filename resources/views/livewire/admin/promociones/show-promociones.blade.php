                ...
                <x-simple-card class="w-72 relative flex flex-col gap-2 justify-between">
                    <div class="w-full">
                        ...
                        <div class="p-1">
                            ...
                            @if (mi_empresa()->uselistprice)
                                @php
                                    $precios = \App\Helpers\GetPrice::getPriceProducto($item->producto, $pricetype_id, null)->getData();
                                @endphp
                                ...
                                @if ($precios->existsrango || $precios->pricemanual > 0)
                                    ...
                                    @else
                                        <x-label-price>
                                            S/.
                                            {{ number_format(!is_null($precios->pricemanual) ? $precios->pricemanual : $precios->pricewithdescount ?? $precios->pricesale, $precios->decimal, '.', ', ') }}
                                            <small>SOLES</small>
                                        </x-label-price>
                                    @endif
                                @else
                            ...
                            @else
                                @php
                                    $precios = \App\Helpers\GetPrice::getPriceProducto($item->producto, null, null)->getData();
                                @endphp

                                <x-span-text text="CALCULAR PRECIOS SIN LISTA DE PRECIOS"
                        ...
                        <div class="w-full flex flex-col gap-1 p-1">
                            @foreach ($item->itempromos as $oferta)
                                <div class="w-full flex gap-2 bg-body rounded relative">
                                    <div
                                        class="block rounded overflow-hidden flex-shrink-0 w-16 h-auto shadow relative hover:shadow-lg cursor-pointer">
                                        @if (count($oferta->producto->defaultImage))
                                            <img src="{{ asset('storage/productos/' . $oferta->producto->defaultImage->first()->url) }}"
                                                alt=""
                                                class="w-full h-full object-scale-down hover:scale-125 duration-150">
                                        @else
                                    ...
                                    <div class="p-1">
                                        ...
                                        @php
                                            $precioscombo = \App\Helpers\GetPrice::getPriceProducto($oferta->producto, $pricetype_id, null, $oferta->descuento)->getData();
                                        ...
                                        @if ($precioscombo->existsrango || $precioscombo->pricemanual > 0)
                                            <h1
