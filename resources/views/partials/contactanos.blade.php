<x-app-layout>
    <div class="min-h-screen flex flex-col items-center md:justify-center">
        <div
            class="w-full text-sm text-colorsubtitleform md:max-w-4xl p-3 md:p-6 bg-fondominicard shadow-md md:rounded-xl">
            {{-- <h1 class="text-colortitle">¡Escríbenos!</h1> --}}

            <div class="w-full flex flex-col md:flex-row gap-5 text-colorlabel">
                <div class="w-full md:w-[40%] flex-shrink-0 flex flex-col gap-2 justify-between">
                    <h2 class="text-4xl lg:text-5xl font-bold text-colortitle text-center md:text-start">
                        Escríbenos!</h2>
                    <div class="text-colorsubtitleform text-center md:text-start">
                        Estamos para atenderte al instante.
                    </div>

                    <div class="text-center">
                        <svg class="w-full h-40 md:h-auto" xmlns="http://www.w3.org/2000/svg"
                ...
                <form class="w-full flex-1" method="POST" action="{{ route('contactanos.store') }}"
                    id="form_contact">
                    @csrf
                    <div>
                        <x-label value="NOMBRE COMPLETO" class="font-semibold text-sm mb-2" />
                        <x-input class="w-full block p-3 !rounded-xl" name="name"
                            value="{{ old('name') ?? '' }}" />
                        <x-jet-input-error for="name" />
                    </div>
                    <div class="mt-5">
                        <x-label value="EMAIL" class="font-semibold text-sm mb-2" />
                        <x-input class="w-full block p-3 !rounded-xl" name="email" type="email"
                            value="{{ old('email') ?? '' }}" />
                        <x-jet-input-error for="email" />
                    </div>
                    <div class="mt-5">
                        <x-label value="MENSAJE" class="font-semibold text-sm mb-2" />
                        <x-text-area class="w-full block p-3 !rounded-xl" name="descripcion" rows="6">
                            {{ old('descripcion') ?? '' }}
                        </x-text-area>
                    ...
                    <div class="mt-5 text-center">
                        <x-button-web type="submit" :text="__('Send message')" class="w-full mx-auto sm:max-w-xs block">
                        </x-button-web>
                    </div>
                </form>
            </div>
